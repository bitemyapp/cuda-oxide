...
fn rename_block(
    ...
    for &op in &ops {
        ...
        // Looking at every allocation for every promotable operation made this
        // walk quadratic in large generated kernels. The allocation pointer's
        // use-list gives us the exact candidates that can affect this operation.
        for cand in candidates_by_user.get(&op).into_iter().flatten() {
            let ptr = cand.alloc_info.ptr;
...
pub fn mem2reg(root: Ptr<Operation>, ctx: &mut Context) -> Result<OptStatus> {
    ...
    for (region, mut alloc_candidates) in by_region {
        ...
        // Build the inverse of Value::uses once. The interface contract requires
        // promotion_kind to return NonPromotableUse when an operation does not
        // use the queried pointer, so omitting those unrelated queries preserves
        // behavior while changing the rename walk from O(ops * allocations) to
        // O(actual allocation uses).
        let mut candidates_by_user: FxHashMap<Ptr<Operation>, Vec<AllocCandidate>> =
            FxHashMap::default();
        for cand in &alloc_candidates {
            let mut users = FxHashSet::default();
            for r#use in cand.alloc_info.ptr.uses(ctx) {
                if users.insert(r#use.user_op()) {
                    candidates_by_user
                        .entry(r#use.user_op())
                        .or_default()
                        .push(cand.clone());
